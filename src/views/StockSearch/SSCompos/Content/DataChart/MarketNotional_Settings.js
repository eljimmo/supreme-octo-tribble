  ...
  const series = [
    {
      ...
      data: data.map((minutebar, index) => {
        ...
        if (minutebar.marketNotional === null && index !== 0) {
          minutebar.marketNotional = data[index - 1].marketNotional;
        }
        const datetime =
        ...
        return [datetime, minutebar.marketNotional];
      }),
    },
  ];
  return series;
...
const setOptions = (priceChange) => {
  ...
  return {
    ...
    title: {
      text: 'Day Market Notional Chart',
      align: 'center'
    },
    grid: {
      show: true,
